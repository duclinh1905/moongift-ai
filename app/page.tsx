...
export default async function HomePage() {
  const [products, caseStudies, blogPosts, landingPages] = await Promise.all([getPublishedProducts(), getCaseStudies(), getBlogPosts(), getLandingPages()]);
  ...
  return (
    <>
      ...
      <main>
        <section className="border-b bg-[radial-gradient(circle_at_top_left,hsl(var(--secondary)),transparent_35%),linear-gradient(135deg,hsl(var(--background)),hsl(var(--muted)))]">
          <div className="container-page grid min-h-[calc(100svh-4rem)] items-center gap-10 py-16 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <h1 className="mt-6 max-w-4xl text-5xl font-bold leading-tight sm:text-7xl">Turn corporate gifting into a growth channel.</h1>
              <p className="mt-6 max-w-2xl text-lg text-muted-foreground">MoonGift combines curated gift collections, SEO landing pages, AI recommendations, and sales CRM workflows for high-value seasonal gifting programs.</p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button asChild size="lg"><Link href="#quote">Request quote <ArrowRight className="size-4" /></Link></Button><Button asChild variant="outline" size="lg"><Link href="/industries">Explore industries</Link></Button></div>
            </div>
        ...
        <section className="container-page py-14"><div className="mb-6 flex items-end justify-between gap-4"><div><h2 className="text-2xl font-bold">Featured Collections</h2><p className="mt-2 text-muted-foreground">Curated gift sets for employees, partners, and VIP clients.</p></div><Button asChild variant="outline"><Link href="/catalog">All products</Link></Button></div><div className="grid gap-4 md:grid-cols-4">{products.slice(0, 4).map((product) => <Card key={product.id}><CardHeader><CardTitle>{product.name}</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">{product.description}</p><div className="mt-4 font-semibold">From {currency(product.price_from)}</div><Button asChild variant="outline" className="mt-4"><Link href={`/products/${product.slug}`}>View product</Link></Button></CardContent></Card>)}</div></section>
