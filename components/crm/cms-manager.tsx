...
function defaultPayload(type: CmsType, formData: FormData) {
  ...
  const status = String(statusValues.at(-1) || "draft");
  ...
  if (type === "categories") {
    return { slug, name: title, description: String(formData.get("description") || ""), status, seo_title, seo_description, published_at: status === "published" ? new Date().toISOString() : null };
  }
  if (type === "collections") {
    return { slug, title, description: String(formData.get("description") || seo_description), featured_image_url, status, seo_title, seo_description, published_at: status === "published" ? new Date().toISOString() : null };
  }
  if (type === "products") {
  ...
  if (type === "blog_posts") {
    return { slug, title, excerpt: String(formData.get("excerpt") || seo_description), content: String(formData.get("content") || ""), category: String(formData.get("category") || "Guides"), tags: String(formData.get("tags") || "").split(",").map((tag) => tag.trim()).filter(Boolean), author: String(formData.get("author") || "MoonGift Editorial Team"), featured_image_url, status, seo_title, seo_description, published_at: status === "published" ? new Date().toISOString() : null };
  }
  if (type === "case_studies") {
    return { slug, title, customer: String(formData.get("customer") || "Customer"), industry: String(formData.get("industry") || "B2B"), challenge: String(formData.get("challenge") || ""), solution: String(formData.get("solution") || ""), results: String(formData.get("results") || ""), featured_image_url, status, seo_title, seo_description, published_at: status === "published" ? new Date().toISOString() : null };
  }
  if (type === "landing_pages") {
    return { slug, title, industry: String(formData.get("industry") || title), hero: String(formData.get("excerpt") || ""), body: String(formData.get("content") || ""), faqs: parseFaqs(formData.get("faqs")), featured_image_url, status, seo_title, seo_description, published_at: status === "published" ? new Date().toISOString() : null };
  }
  ...
  if (type === "banners") {
    return { slug, title, body: String(formData.get("content") || ""), cta_label: String(formData.get("cta_label") || "Request quote"), cta_href: String(formData.get("cta_href") || "/#quote"), image_url: featured_image_url, placement: String(formData.get("placement") || "homepage"), status, published_at: status === "published" ? new Date().toISOString() : null };
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[18rem_1fr]">
      ...
      <section className="grid gap-6">
        ...
        <form action={submit} className="grid gap-4 rounded-lg border bg-card p-5">
          ...
          <Textarea name="faqs" placeholder={'Landing page FAQs as JSON or one per line: Question | Answer'} defaultValue={Array.isArray(editing?.faqs) ? JSON.stringify(editing.faqs, null, 2) : ""} />
          ...
          <div className="grid gap-3 md:grid-cols-3"><Field name="customer" label="Customer" defaultValue={String(editing?.customer ?? "")} /><Field name="challenge" label="Challenge" defaultValue={String(editing?.challenge ?? "")} /><Field name="results" label="Results" defaultValue={String(editing?.results ?? "")} /><Field name="author" label="Author" defaultValue={String(editing?.author ?? "")} /><Field name="cta_label" label="CTA label" defaultValue={String(editing?.cta_label ?? "")} /><Field name="cta_href" label="CTA href" defaultValue={String(editing?.cta_href ?? "")} /><Field name="placement" label="Banner placement" defaultValue={String(editing?.placement ?? "homepage")} /></div>
          ...
          <div className="flex flex-wrap gap-2"><Button disabled={isPending}>{isPending ? "Saving..." : "Save"}</Button><Button type="submit" name="status" value="published" variant="outline" disabled={isPending}>Publish</Button><Button type="submit" name="status" value="draft" variant="outline" disabled={isPending}>Save draft</Button>{editing ? <Button type="button" variant="outline" onClick={() => setEditing(null)}>Cancel</Button> : null}</div>
        </form>
